...
- A sample is discarded if it sits more than **25 %** away from the running
  median of the epoch's samples so far. One strange print does not move the
  average.
- A failed read is skipped, not counted as zero.

At the boundary the **time-weighted price** — the TWAP — is the arithmetic mean
of the samples that survived the filter.
...
|---|---|---|
| above **1.01** | **Bright** — expansion | new GOLD is minted and scattered as chests |
...
2. **Fewer than three samples survived the filter.** A broken or unreadable price
   feed cannot be used to mint, and it cannot be used to open the bond market
   either.
...
When the TWAP is above the ceiling, the Keeper mints. How much is the smallest of
three numbers: how far above the peg the price actually is, a tier that shrinks
as the token grows, and a hard cap.
