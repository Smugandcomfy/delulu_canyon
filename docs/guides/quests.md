...
```mermaid
flowchart LR
    E["Eledrin, in the Grove:<br/>one jewel, both, or neither"] --> Z["Elder Zaraphon, in the Sanctum:<br/>rekindle the Flame"]
    Z --> W["The Wizard, at the top of the tower:<br/>the reading"]
    K["a chest on the Hill:<br/>the Ancient Key"] --> D{"the tower door"}
    P["Pippin, in the Hearth:<br/>the Loaf of Friendship"] --> D
    D --> W
    W --> S["Elder Zaraphon again:<br/>the Soul Gem"]
    S --> V["the shaft, the dragon,<br/>and the chest that was never all found"]
```
...
```mermaid
flowchart LR
    S["skeletons, and what the halls drop"] --> D[("bone dust")]
    C["curios, given to Quill Amberly"] --> D
    F["what comes up on a fishing line"] --> D
    T["hides and fangs at the folk's tables"] --> D
    D --> P["potions at a cauldron"]
    D --> W["cloaks, and a bone charm"]
    D --> R["a rod from Marrow Quill"]
    D --> E["an egg hurried along"]
```
